...
const Shop: React.FC = () => {
  ...
  // Debounce search and price slider changes to avoid excessive API calls
  const debouncedSearch = useDebounce(filters.search, 500);
  const debouncedMinPrice = useDebounce(filters.minPrice, 500);
  const debouncedMaxPrice = useDebounce(filters.maxPrice, 500);

  // Optimization: Fetch filtered products from server instead of client-side filtering
  useEffect(() => {
    let isMounted = true;
    ...
    const fetchProducts = async () => {
      ...
      // Create a filter object with debounced values
