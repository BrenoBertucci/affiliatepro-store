...
export const ProductService = {
  ...
  // Optimized: Server-side filtering to reduce payload size and client computation
  getFiltered: async (filters: { category: string | null, minPrice: number, maxPrice: number, search: string }, sortOption: string): Promise<Product[]> => {
    ...
    if (filters.search) {
      ...
      // Note: for better performance on large datasets, Full Text Search (textSearch) should be configured in Supabase
      const searchTerm = filters.search.trim().replace(/,/g, ' '); // simple sanitization
      if (searchTerm) {
