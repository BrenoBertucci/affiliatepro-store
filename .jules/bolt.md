...
## 2024-05-27 - Supabase Search Sanitization
**Learning:** When using Supabase's `.or()` filter for search across multiple columns (e.g., name and description), commas in the search string can break the query syntax.
**Action:** Always sanitize user input (e.g., replacing commas with spaces) before passing it to `.or()` clauses in Supabase queries.
